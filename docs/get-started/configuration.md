...
When backups are enabled, an operation that promises a safety backup must complete that backup before it deletes, replaces, imports, cleans, or regenerates translations. A storage or creation failure aborts the requested destructive operation. An empty current translation state is a successful no-op, while disabling backups preserves the explicit no-backup policy. CSV and PHP imports continue to respect `backupOnImport` and their per-import backup choice.
